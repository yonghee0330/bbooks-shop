...
import { getBbooksPicks } from '../../utils/supabase'
...
export default function BbooksPicksTab() {
  ...
  useEffect(() => {
    getBbooksPicks('2024-12').then(data => {
      setPicks(data.length > 0 ? data : BBOOKS_PICKS)
      setLoading(false)
    })
  }, [])
...
export function CuratedCard({ book, color, curatorName, added, onAdd }) {
  return (
    <div
      style={{ background: 'white', border: '1px solid #e2dbd0', borderRadius: 8, overflow: 'hidden', transition: 'transform 0.2s, box-shadow 0.2s' }}
    ...
    >
      ...
      <div style={{ padding: '0 1rem 1rem' }}>
        ...
        <div style={{ fontSize: '0.95rem', fontWeight: 600, lineHeight: 1.4, marginBottom: 3 }}>{book.book || book.book_title}</div>
